...
def generate_fsr(t, pavg, decay_rate=0.02):
    ...
    dp = np.gradient(pavg)
    ddp = np.gradient(dp)
    motion = np.abs(dp) + 0.5 * np.abs(ddp)

    motion_threshold = np.std(motion) * 0.3
    is_static = motion < motion_threshold

    # ===== GLOBAL STATIC FILL (CRITICAL FIX) =====
    for j in range(n):
        if is_static[j]:
            base_load = np.random.uniform(2200, 3500)

            fsr1[j] = base_load + np.random.uniform(0, 250)  # heel bias ↑
            fsr2[j] = base_load + np.random.uniform(-150, 150)
            fsr3[j] = base_load - np.random.uniform(0, 300)
    ...
    for i in range(len(events) - 1):
        ...
        for j in range(start, end):
            ...
            # DO NOT overwrite static regions
            if is_static[j]:
                continue
            ...
            if phase < 0.15:
                ...
                fsr1[j] = np.random.uniform(2500, 3800) * amp_scale
...
if __name__ == "__main__":
    ...
    imu_file = "IMU_data_20.mat"   # change as needed
